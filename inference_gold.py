...
def join_all_feat(root, feat_file):
  with open(feat_file, 'w') as f:
    for video in range(1098):
      folder = root + '%04d\\' % video
      print (folder)
      if not os.path.exists(folder):
        continue
      for file in [file for file in os.listdir(folder) if file.endswith('.txt')]:
        ...
        f.write(('%04d/' % video) + file + '\t' + features + '\n')

def loadgold(max_frames = 300, num=None):
  root = r'E:\topic_golden_set\frames\\'
  feat_file = r'E:\topic_golden_set\all_feature.txt'
  #if not os.path.isfile(feat_file):
  #  join_all_feat(root, feat_file)
  video_id_batch_val = [] #video_id_batch_val[:100]
  video_batch_val = [] #video_batch_val[:100]
  video_batch_temp = []
  video_id_temp = ''
  with open(feat_file, 'r') as f:
    for line in f:
      ...
      if (len(t) != 2):
        continue
      video_id = t[0].split('/')[0]
      if video_id_temp != video_id and video_id_temp != '':
        print (video_id_temp, len(video_batch_temp))
        video_id_batch_val.append(video_id_temp)
        video_batch_val.append(video_batch_temp)
        video_batch_temp = []

      if (num != None and len(video_id_batch_val) >= num):
        break
      video_id_temp = video_id
      video_batch_temp.append([float(f) for f in t[1].split(',')])
  if (len(video_batch_temp) > 0):
    video_id_batch_val.append(video_id_temp)
    video_batch_val.append(video_batch_temp)
   
  num_frames_batch_val = [] #num_frames_batch_val[:100]
  for i in range(len(video_batch_val)):
    l = len(video_batch_val[i])
    if l > max_frames:
      num_frames_batch_val.append(max_frames)
      video_batch_val[i] = [video_batch_val[i][int(j)] for j in numpy.linspace(0,l-1,max_frames)]
    else:
      num_frames_batch_val.append(l)
      video_batch_val[i] = video_batch_val[i] + [[0]*1024] * (max_frames-l)
  return video_id_batch_val, numpy.array(video_batch_val), numpy.array(num_frames_batch_val)
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k):
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess, gfile.Open(out_file_location, "w+") as out_file:
    ...
    checkpoint_file = os.path.join(FLAGS.train_dir, "deploy_model", "deploy_model") #"inference_model", "inference_model")
    ...
    try:
      ...
      video_id_batch_val, video_batch_val, num_frames_batch_val = loadgold(30)
      pad = input_tensor.shape[2] - video_batch_val.shape[2]
      if (pad > 0):
        video_batch_val = numpy.pad(video_batch_val, ((0,0),(0,0),(0,pad)), 'constant')
