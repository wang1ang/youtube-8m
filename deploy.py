...
def get_input_evaluation_tensors(reader,
  ...
  with tf.name_scope("deploy_input"):
...
def build_graph(#reader,
  ...
  video_id_batch = tf.placeholder(tf.string, shape=(None))
  model_input_raw = tf.placeholder(tf.float32, shape=(None, 30, 1024))
  labels_batch = tf.placeholder(tf.bool, shape=(None, 3800))
  num_frames = tf.placeholder(tf.int32, shape=(None))
  ...
  with tf.variable_scope("tower"):
    result = model.create_model(model_input,
                                ...
                                vocab_size=3862, #reader.num_classes,
                                #labels=labels_batch,
...
def evaluation_loop(
  ...
  with tf.Session() as sess:
    ...
    if latest_checkpoint:
      ...
      # Save model
      saver.save(sess, os.path.join(FLAGS.train_dir, "deploy_model", "deploy_model"))
    else:
...
def evaluate():
  ...
  with tf.Graph().as_default():
    ...
    summary_writer = tf.summary.FileWriter(
        os.path.join(FLAGS.train_dir, "deploy"),
